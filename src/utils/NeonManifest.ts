...
export function parseManifest(manifestString: NeonManifest): boolean {
  ...
  if (
    context === 'https://ddmal.ca/Neon/contexts/1/manifest.jsonld'
  ) {
    return true;
  }
  if (
    context[0] === NeonContext[0] &&
    ...
    context[1]['mei_annotations']['@id'] ===
      NeonContext[1]['mei_annotations']['@id'] &&
    context[1]['mei_annotations']['@type'] ===
      NeonContext[1]['mei_annotations']['@type'] &&
    context[1]['mei_annotations']['@container'] ===
      NeonContext[1]['mei_annotations']['@container']
  ) {
    return true;
  } else {
